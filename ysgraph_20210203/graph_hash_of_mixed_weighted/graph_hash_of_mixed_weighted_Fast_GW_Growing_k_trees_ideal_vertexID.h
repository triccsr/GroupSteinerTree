...
typedef PairingHeapYS_min<double, int> PairingHeap_1; // define data types in heap
#include <PairingHeapYS_with_offset.h>
typedef PairingHeapYS_with_offset<double, int> PairingHeap_2; // define data types in heap

graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_Fast_GW_Growing_k_trees_ideal_vertexID
(graph_hash_of_mixed_weighted& input_graph, string return_content, int k) {

	if (return_content != "active" && return_content != "active_and_inactive") {
		std::cout << "Error in graph_hash_of_mixed_weighted_Fast_GW_Growing_k_trees_ideal_vertexID: " <<
			"return_content != active && return_content != active_and_inactive" << std::endl;
		exit(1);
	}
	...
	std::vector<PairingHeap_2::ItemHandle> handle_ep(ep_num); // store the handle for each edge part
	vector<PairingHeap_2> C_ep_PQ(N);
	...
	std::vector<PairingHeap_1::ItemHandle> handle_Cevent(N), handle_Eevent(N);
	PairingHeap_1 C_event_PQ, E_event_PQ;
	...
	pair<int, int> edge;
	while (Active_C_num > k) // stop the loop when there is only k active clusters left
	{
...
#pragma endregion graph_hash_of_mixed_weighted_Fast_GW_Growing_k_trees_ideal_vertexID 
