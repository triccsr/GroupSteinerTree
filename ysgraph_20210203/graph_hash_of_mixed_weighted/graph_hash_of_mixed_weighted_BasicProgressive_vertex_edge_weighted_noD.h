...
/*
Removing D does not make Basic faster than DPBF
*/
#include <queue>
...
pair<std::unordered_map<int, double>, std::unordered_map<int, int>> graph_hash_of_mixed_weighted_BasicProgressive_find_LWPs_to_g
...
std::unordered_map<int, pair<std::unordered_map<int, double>, std::unordered_map<int, int>>> graph_hash_of_mixed_weighted_BasicProgressive_find_LWPs
	...
	*/
	...
	while (Q_T.size() > 0) { // at most 2^|Gamma|*V loops
		...
		for (auto it = uncovered_groups[p].begin(); it != uncovered_groups[p].end(); it++) {
			...
			int v_start = v;
			int pre = LWPs_to_groups[g_id].second[v_start]; // LWPs_to_groups[g_id].second is the predecessor index
			while (v_start != pre) {
				...
				v_start = pre;
				pre = LWPs_to_groups[g_id].second[v_start];
			}
		}
		...
		for (auto it = v_adjs.begin(); it != v_adjs.end(); it++) {
			...
			if (grow_tree_cost > best_cost + 1e-10) { // 1e-10 is error
		...
		for (auto it = non_overlapped_group_sets_IDs[p1].begin(); it != non_overlapped_group_sets_IDs[p1].end(); it++) {
			...
			if (trees[v].count(p2) > 0) { // otherwise Basic does not find feasible solutions
				int p1_cup_p2 = p1 + p2;
