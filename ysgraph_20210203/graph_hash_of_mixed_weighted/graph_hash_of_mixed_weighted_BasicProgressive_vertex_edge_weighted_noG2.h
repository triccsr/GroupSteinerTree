...
#pragma region
std::unordered_map<int, int> graph_hash_of_mixed_weighted_BasicProgressive_find_LWPs_to_g
...
std::unordered_map<int, std::unordered_map<int, int>> graph_hash_of_mixed_weighted_BasicProgressive_find_LWPs
(graph_hash_of_mixed_weighted& input_graph, graph_hash_of_mixed_weighted& group_graph, std::unordered_set<int>& cumpulsory_group_vertices, double lambda) {
	...
	std::unordered_map<int, std::unordered_map<int, int>> LWPs_to_groups;
	...
	*/
	...
	while (Q_T.size() > 0) { // at most 2^|Gamma|*V loops
		...
		for (auto it = uncovered_groups[p].begin(); it != uncovered_groups[p].end(); it++) {
			...
			int v_start = v;
			int pre = LWPs_to_groups[g_id][v_start]; // LWPs_to_groups[g_id].second is the predecessor index
			while (v_start != pre) {
				...
				v_start = pre;
				pre = LWPs_to_groups[g_id][v_start];
			}
		}
		//cout << "feasible_solu size: " << feasible_solu.hash_of_vectors.size() << endl;
		//graph_hash_of_mixed_weighted_print(feasible_solu);
		...
		//graph_hash_of_mixed_weighted_print(feasible_solu);
		//cout << "feasible_solu_cost " << feasible_solu_cost << endl;
