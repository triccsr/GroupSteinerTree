...
graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_Fast_GW_Growing_tree_check_boundary(graph_hash_of_mixed_weighted& input_graph, string return_content,
	bool& cross_boundary, std::unordered_set<int>& Gsub_boundary_vertices) {
	...
	*/
	...
	for (auto it = hash_of_v.begin(); it != hash_of_v.end(); it++) {
		if (Gsub_boundary_vertices.count(*it) > 0) {
			cross_boundary = true;
	...
	if (return_content == "tree") {
		...
		for (int i = 0; i < merged_edges.size(); i++) {
			if (Clusters[Union_Find_hash_find(merged_edges[i].first, Union_Find_hash_elements)].activity == true) { // an edge in active cluster
