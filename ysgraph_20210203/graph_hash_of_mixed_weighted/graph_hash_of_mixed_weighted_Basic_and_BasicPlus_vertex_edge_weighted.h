...
#include <graph_hash_of_mixed_weighted/graph_hash_of_mixed_weighted_lowest_weight_paths.h>

#pragma region
pair<std::unordered_map<int, int>, std::unordered_map<int, double>> graph_hash_of_mixed_weighted_BasicProgressive_find_LWPs_to_g
(graph_hash_of_mixed_weighted& group_graph, graph_hash_of_mixed_weighted& input_graph, int g_vertex, double lambda) {
	...
	graph_hash_of_mixed_weighted_lowest_weight_paths_source_to_all_lambda(input_graph, g_vertex, distances, predecessors, lambda);
...
std::unordered_map<int, pair<std::unordered_map<int, int>, std::unordered_map<int, double>>> graph_hash_of_mixed_weighted_BasicProgressive_find_LWPs
(graph_hash_of_mixed_weighted& input_graph, graph_hash_of_mixed_weighted& group_graph, std::unordered_set<int>& cumpulsory_group_vertices, double lambda) {
	...
	std::unordered_map<int, pair<std::unordered_map<int, int>, std::unordered_map<int, double>>> LWPs_to_groups;
	for (auto it = cumpulsory_group_vertices.begin(); it != cumpulsory_group_vertices.end(); it++) {
		int g_vertex = *it;
		LWPs_to_groups[g_vertex] = graph_hash_of_mixed_weighted_BasicProgressive_find_LWPs_to_g(group_graph, input_graph, g_vertex, lambda);
...
graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_ProgressiveReturn(graph_hash_of_mixed_weighted& input_graph, graph_hash_of_mixed_weighted& group_graph,
	std::unordered_set<int>& cumpulsory_group_vertices, double lambda, double maximum_return_app_ratio) {
	...
	it is slower than DPBF, and it's not because it finds LWPs (this is just a small portion of time), it's because it iteratively finds feasible solutions;

	it has similar speed with DPBF when T>10 in tiny graphs, lambda does not matter much in this comparison, and finding LWPs does not matter much as well;

	cost function: (1 - lambda) * vertex_weights + lambda * edge_weights */


	if (cumpulsory_group_vertices.size() >= 20) {
		std::cout << "cumpulsory_group_vertices.size() is too large for graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_ProgressiveReturn!" << std::endl;
		exit(1);
	}
	...
	*/
	...
	while (Q_T.size() > 0) { // at most 2^|Gamma|*V loops
		...
		for (auto it = uncovered_groups[p].begin(); it != uncovered_groups[p].end(); it++) {
			feasible_solu_cost = feasible_solu_cost + LWPs_to_groups[*it].second[v] - cost_v;
		}
		if (feasible_solu_cost < best_cost) { // best_cost is also updated in merge!
			...
			for (auto it = uncovered_groups[p].begin(); it != uncovered_groups[p].end(); it++) {
				...
				int v_start = v;
				int pre = LWPs_to_groups[g_id].first[v_start]; // LWPs_to_groups[g_id].second is the predecessor index
				while (v_start != pre) {
					...
					v_start = pre;
					pre = LWPs_to_groups[g_id].first[v_start];
				}
			}
			best_solu = graph_hash_of_mixed_weighted_MST_postprocessing_no_whole_graph(feasible_solu);
			best_cost = graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_treeCost(best_solu, lambda);
		}

		/*
		the above pruning process is not the same as the pruning process in the pseudo code in the 2016 SIGMOD paper;

		the above pruning process is from the actual codes provided by the author of the 2016 paper (which means that the experiments in the 2016 paper are based on the above pruning process);

		the pruning process in the pseudo code in the 2016 SIGMOD paper has been commented out as follows.

		the above pruning process is much faster than the following pruning process, since we do not merge minimum weight paths into every T(v,p) in the above pruning process;

		since the above pruning process is much faster, we use the above pruning process
		*/
		...
		//	}
		//}
		//feasible_solu = graph_hash_of_mixed_weighted_MST_postprocessing_no_whole_graph(feasible_solu);
		//double feasible_solu_cost = graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_treeCost(feasible_solu, lambda);
		//if (feasible_solu_cost < best_cost) { // best_cost is also updated in merge!			
		//	best_solu = feasible_solu;
		...
		for (auto it = non_overlapped_group_sets_IDs[p1].begin(); it != non_overlapped_group_sets_IDs[p1].end(); it++) {
			...
			if (D.count(handle_ID) > 0) { // only merge optimal T(v,p2), this is recorded in the pseudo code of 2016 paper
				...
				if (p1_cup_p2 == group_sets_ID_range) {
					...
					best_solu = graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_build_tree(v, p1_cup_p2, input_graph, trees);
				}
	...
	std::cout << "graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_ProgressiveReturn did not find a feasible solution!" << std::endl;
	getchar();
	exit(1);
...
graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_BasicProgressivePlus_vertex_edge_weighted_ProgressiveReturn(graph_hash_of_mixed_weighted& input_graph, graph_hash_of_mixed_weighted& group_graph,
	std::unordered_set<int>& cumpulsory_group_vertices, double lambda, double maximum_return_app_ratio) {

	/* this function returns the first found feasible solution that has an error ratio not larger than maximum_return_app_ratio.

	this function is Basic+, i.e., using the One-label lower bound to enhance Basic

	cost function: (1 - lambda) * vertex_weights + lambda * edge_weights */


	if (cumpulsory_group_vertices.size() >= 15) {
		std::cout << "cumpulsory_group_vertices.size() is too large for graph_hash_of_mixed_weighted_BasicProgressivePlus_vertex_edge_weighted_ProgressiveReturn!" << std::endl;
		exit(1);
	}


	/*finding lowest-weighted paths from groups to vertices; time complexity: O(|T||E|+|T||V|log|V|); return {g_ID, {predecessors, distances}} */
	std::unordered_map<int, pair<std::unordered_map<int, int>, std::unordered_map<int, double>>> LWPs_to_groups =
		graph_hash_of_mixed_weighted_BasicProgressive_find_LWPs(input_graph, group_graph, cumpulsory_group_vertices, lambda);

	/*initialize Q*/
	boost::heap::fibonacci_heap<graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_min_node> Q_T; // min queues of trees
	std::unordered_map<string, handle_graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_min_node> Q_T_handles; // key is string "v_p" ("vertex_ID" + "_" + "group set ID"); Q_T_handles only contain keys that are in Q_T
	...
	/*initialize uncovered_groups;  <p, <uncovered_groups>>;  time complexity: O(|Gamma|*2^|Gamma|)*/
	std::unordered_map<int, std::unordered_set<int>> uncovered_groups = graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_uncovered_groups(group_sets_ID_range, cumpulsory_group_vertices);
	...
	while (Q_T.size() > 0) { // at most 2^|Gamma|*V loops
		...
		*/
		...
		/*One-label lower bound pruning*/
		double One_label_LB = 0;
		for (auto it = uncovered_groups[p].begin(); it != uncovered_groups[p].end(); it++) {
			double cost = LWPs_to_groups[*it].second[v] - cost_v;
			if (One_label_LB < cost) {
				One_label_LB = cost;
			}
		}
		if (top_node.priority_value + One_label_LB > best_cost + 1e-8) { // cannot have >= here, as otherwise optimal solution cannot be found; same as 1e-8, due to precision limitation
			continue;
		}
	...
	std::cout << "graph_hash_of_mixed_weighted_BasicProgressivePlus_vertex_edge_weighted_ProgressiveReturn did not find a feasible solution!" << std::endl;
	getchar();
	exit(1);
...
graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_BasicProgressive_vertex_edge_weighted_ProgressiveReturn_report_memory(graph_hash_of_mixed_weighted& input_graph, graph_hash_of_mixed_weighted& group_graph,
	std::unordered_set<int>& cumpulsory_group_vertices, double lambda, double maximum_return_app_ratio, double& peak_memory) {
...
graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_BasicProgressivePlus_vertex_edge_weighted_ProgressiveReturn_report_memory(graph_hash_of_mixed_weighted& input_graph, graph_hash_of_mixed_weighted& group_graph,
	std::unordered_set<int>& cumpulsory_group_vertices, double lambda, double maximum_return_app_ratio, double& peak_memory) {
