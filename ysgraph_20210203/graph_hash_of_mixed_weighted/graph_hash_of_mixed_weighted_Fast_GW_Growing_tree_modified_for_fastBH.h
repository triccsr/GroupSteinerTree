...
/*the following class is also in #include <graph_hash_of_mixed_weighted/graph_hash_of_mixed_weighted_Fast_GW_Growing_tree.h>, so it's omitted here*/
...
graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_Fast_GW_Growing_tree_modified_for_fastBH(graph_hash_of_mixed_weighted& input_graph) {
	...
	*/
	...
	pair<int, int> edge;
	double Active_C_num_1_glocal_time = INT_MAX;
	while (Active_C_num > 1) // stop the loop when there is only 1 active clusters left
	{
		...
		//if (Active_C_num == 1 && Active_C_num_1_glocal_time == INT_MAX) { // the first time Active_C_num == 1
		//	Active_C_num_1_glocal_time = Global_time;
		//}
		//if (Global_time >= 1 * Active_C_num_1_glocal_time) { // a dual method
		//	break;
		//}
	...
	/*below are two ways to return the subgraph built by all vertices that have been in active clusters:
	
	1. using graph_hash_of_mixed_weighted_extract_subgraph_for_a_hash_of_vertices
	
	2. removing vertices from a copy of input_graph, and then return the copy
	
	clearly, the first way is fast; however, the returned subgraph in the first way may have different orders of vertices from input_graph,
	while the returned subgraph in the second way has the same orders of vertices with input_graph;
	
	Since all edge weights in bump hunting scenarios are equal, the orders of vertices play an important role in what solutions FGW produces.
	Thus, if we use the first way, then A_MIRROR may have worse solutions than MIRROR, and if we use the second way, then
	A_MIRROR has the same solutions with MIRROR*/


	return graph_hash_of_mixed_weighted_extract_subgraph_for_a_hash_of_vertices(input_graph, hash_of_v);

	//graph_hash_of_mixed_weighted output_graph = graph_hash_of_mixed_weighted_copy_graph(input_graph);
	//for (auto it = input_graph.hash_of_vectors.begin(); it != input_graph.hash_of_vectors.end(); it++) {
	//	if (hash_of_v.count(it->first) == 0) {
	//		graph_hash_of_mixed_weighted_remove_vertex(output_graph, it->first);
	//	}
	//}
	//return output_graph;
