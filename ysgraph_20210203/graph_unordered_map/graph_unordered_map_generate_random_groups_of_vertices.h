...
void graph_unordered_map_generate_random_groups_of_vertices(int G, int g_size_min, int g_size_max,
	graph_unordered_map& input_graph, int group_vertex_start_ID,
	std::unordered_set<int>& generated_group_vertices, graph_unordered_map& generated_group_graph) {
	...
	for (auto it = input_graph.begin(); it != input_graph.end(); it++) {
		int v = it->first;
		graph_unordered_map_add_vertex(generated_group_graph, v, 0);
		all_v.push_back(v);
	}
	...
	while (add_group_num < G) {
		...
		for (int j = 0; j < linked_vertices.size(); j++) {
			graph_unordered_map_add_edge(generated_group_graph, linked_vertices[j], group_vertex, 1);
		}
