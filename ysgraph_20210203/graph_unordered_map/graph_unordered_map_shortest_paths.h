...
void graph_unordered_map_shortest_paths_source_to_all(graph_unordered_map& input_graph, int source,
	...
	for (auto it = input_graph[source].second.begin(); it != input_graph[source].second.end(); it++) {
		int adj_v = it->first;
		double ec = it->second;
		Q_keys[adj_v] = ec;
		node.index = adj_v;
...
void graph_unordered_map_shortest_paths_source_to_some(graph_unordered_map& input_graph, int source,
