...
graph_unordered_map graph_unordered_map_generate_random_connected_graph(int V, int E, double nw_min, double nw_max,
	...
	/*time complexity: O(|V|)*/
	graph_unordered_map random_graph; // generate vertices
	for (int i = 0; i < V; i++) {
		double new_weight = (double)dist_nw(gen) / precision; // generate nw
		graph_unordered_map_add_vertex(random_graph, i, new_weight);
	}
	...
	if (E == V * (V - 1) / 2) { // complete graphs
		...
		for (int i = 0; i < V; i++) {
			for (int j = 0; j < i; j++) {
				...
				graph_unordered_map_add_edge(random_graph, i, j, new_cost);
			}
		}
	}
	else if (E > V * (V - 1) / 2) {
		...
		while (inside_V.size() < V) {
			...
			double new_cost = (double)dist_ec(gen) / precision; // generate ec
			graph_unordered_map_add_edge(random_graph, v1, v2, new_cost);
		...
		while (edge_num < E) {
			...
			if (random_graph[not_full_vertices[RAND]].second.size() < V - 1) { // this is a vertex without a full degree
				...
				while (added == false) {
					...
					int x = dist_id2(gen);
					int j = unchecked[x];
					if (not_full_vertices[RAND] != j && random_graph[not_full_vertices[RAND]].second.count(j) == 0) {
						// This edge does not exist
						double new_cost = (double)dist_ec(gen) / precision; // generate ec
						graph_unordered_map_add_edge(random_graph, not_full_vertices[RAND], j, new_cost); // add a new edge
						edge_num++;
						added = true;
